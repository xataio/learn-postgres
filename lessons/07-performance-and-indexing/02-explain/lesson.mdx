...
<Check id="status-index-created">
Create the `orders(status)` index above. We'll confirm it exists on your branch — and your re-run `EXPLAIN ANALYZE` should now show a Bitmap Index Scan instead of a Seq Scan.
</Check>
