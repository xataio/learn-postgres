...
The seed is a `docs` table of ~40,000 rows: a `tags` array, a `jsonb` `meta` column, a lopsided `status`, mixed-case `email`, and a `created_at` inserted in time order. Take a look:

<Run>
SELECT id, status, email, tags, meta, created_at FROM docs ORDER BY id LIMIT 5;
</Run>
...
<Run>
EXPLAIN ANALYZE SELECT id FROM docs WHERE meta ? 'priority';
</Run>
...
**Hash** indexes support exactly one operator: `=`. No ranges, no ordering. Since PostgreSQL 10 they're WAL-logged and crash-safe (before that they weren't even replicated), so they're finally usable — but a B-tree also handles `=` well *and* everything else, so hash rarely wins. Reach for it only for equality on a large, wide key where the slightly smaller hash index measurably helps.

<Run>
CREATE INDEX docs_owner_hash ON docs USING hash (owner_id);
</Run>

<Run>
EXPLAIN ANALYZE SELECT id FROM docs WHERE owner_id = 42;
</Run>
